...
from .feature_tokens import usua_feature_list


class FeatureUSUA():
    def __init__(self):
        self.usua_feature_dict = {}
        for idx, t in enumerate(usua_feature_list):
            self.usua_feature_dict[t] = idx
        ...
        self.n_feature = len(usua_feature_list)
    ...
    def filter(self, token):
        ...
        for i in range(len(token)):
            ...
            tok = t.split('.')[0]
            if tok in self.usua_feature_dict.keys():
                result_token.append(self.usua_feature_dict[tok])
        return result_token
